...
(function(Buffer) {
  ...
  runPlatformsTest('_', allPlatformSets.singleBrowserRun, allPlatformSets.allInternetExplorer);
  ...
  function getPlatformsArg(platformSets, callback) {
    var flattened = _.flatten(_.flatten(_.flatten(_.map(platformSets, function(platformSet) {
    ...
    get('info/platforms/webdriver', function(platformsInfo) {
      ...
      _.each(flattened, function(platform) {
        var platformId = platform.join('-').replace('OS X', 'Mac').replace('Windows XP', 'Windows 2003').replace('Windows 7', 'Windows 2008').replace('Windows 8', 'Windows 2012');
        if (seen[platformId]) {
          return;
        }
