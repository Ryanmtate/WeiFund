...
(function(process) {
  ...
  function npmPublish(done) {
    ...
    publish.on('close', (code) => {
      if (code !== 0)
        return fail(`npm publish. Exiting with ${code}.`);
  ...
  gulp.task('release:build', ['build'], () => {
    return gulp.src('./dist').pipe($.git.add()).pipe($.git.commit('chore: build', {args: '-n'}));
  });
  gulp.task('release:bump', () => {
    const type = getType();
    const newVersion = semver.inc(getCurrentVersion(), type);
    return gulp.src('./package.json').pipe($.bump({version: newVersion})).pipe(gulp.dest('./')).pipe($.git.add()).pipe($.git.commit(`chore: release version v${newVersion}`, {args: '-n'})).pipe($.filter('package.json')).pipe($.tagVersion());
  });
  ...
  gulp.task('release', (done) => {
    runSequence('lint', 'test', 'release:build', 'release:bump', 'release:push', 'release:publish', done);
  });
})(require('process'));
