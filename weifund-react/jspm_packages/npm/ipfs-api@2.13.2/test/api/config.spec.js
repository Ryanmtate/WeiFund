...
describe('.config', () => {
  ...
  describe('promise', () => {
    it('.config.{set, get}', () => {
      const confKey = 'arbitraryKey';
      const confVal = 'arbitraryVal';
      return apiClients['a'].config.set(confKey, confVal).then((res) => {
    ...
    it('.config.show', () => {
      return apiClients['c'].config.show().then((res) => {
        expect(res).to.exist;
      });
    });
    it('.config.replace', () => {
      ...
      return apiClients['c'].config.replace(path.join(__dirname, '/../r-config.json')).then((res) => {
        expect(res).to.be.equal(null);
      });
    });
  });
