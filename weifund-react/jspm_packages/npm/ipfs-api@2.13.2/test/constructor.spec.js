...
describe('ipfsAPI constructor tests', () => {
  describe('parameter permuations', () => {
    ...
    it('mutliaddr, opts', (done) => {
      clientWorks(ipfsAPI(apiAddrs.a, {protocol: 'http'}), done);
    });
    it('host, port', (done) => {
      clientWorks(ipfsAPI(apiAddr[2], apiAddr[4]), done);
    });
    it('host, port, opts', (done) => {
      clientWorks(ipfsAPI(apiAddr[2], apiAddr[4], {protocol: 'http'}), done);
    });
  });
});
