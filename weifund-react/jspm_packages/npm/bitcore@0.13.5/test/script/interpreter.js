...
(function(Buffer) {
  ...
  var script_valid = require('../data/bitcoind/script_valid.json!systemjs-json');
  var script_invalid = require('../data/bitcoind/script_invalid.json!systemjs-json');
  var tx_valid = require('../data/bitcoind/tx_valid.json!systemjs-json');
  var tx_invalid = require('../data/bitcoind/tx_invalid.json!systemjs-json');
  ...
  describe('Interpreter', function() {
    ...
    describe('@castToBool', function() {
      it('should cast these bufs to bool correctly', function() {
        Interpreter.castToBool(new BN(0).toSM({endian: 'little'})).should.equal(false);
        Interpreter.castToBool(new Buffer('0080', 'hex')).should.equal(false);
        Interpreter.castToBool(new BN(1).toSM({endian: 'little'})).should.equal(true);
        Interpreter.castToBool(new BN(-1).toSM({endian: 'little'})).should.equal(true);
        var buf = new Buffer('00', 'hex');
        var bool = BN.fromSM(buf, {endian: 'little'}).cmp(BN.Zero) !== 0;
    ...
    describe('#verify', function() {
      ...
      it('should verify these simple transaction', function() {
        ...
        var tx = new Transaction().from(utxo).to(toAddress, 100000).sign(privateKey);
        var inputIndex = 0;
