...
(function(Buffer) {
  ...
  const Wreck = require('../lib/index');
  const internals = {
    payload: new Array(1640).join('0123456789'),
  ...
  describe('request()', () => {
    ...
    it('requests a POST resource with headers', (done) => {
      ...
      server.listen(0, () => {
        Wreck.request('post', 'http://localhost:' + server.address().port, {
    ...
    it('cannot set agent and rejectUnauthorized at the same time', (done) => {
      expect(() => {
        Wreck.request('get', 'https://google.com', {
    ...
    it('applies rejectUnauthorized when redirected', (done) => {
      ...
      server.listen(0, (err) => {
        expect(err).to.not.exist();
        Wreck.request('get', 'https://localhost:' + server.address().port, {
          redirects: 1,
          rejectUnauthorized: false
        }, (err, res) => {
          expect(err).to.not.exist();
    ...
    it('handles redirections from http to https', (done) => {
      ...
      httpsServer.listen(0, (err) => {
        ...
        httpServer.listen(0, (err) => {
          expect(err).to.not.exist();
          Wreck.request('get', 'http://localhost:' + httpServer.address().port, {
            redirects: 1,
            rejectUnauthorized: false
          }, (err, res) => {
            expect(err).to.not.exist();
    ...
    it('calls beforeRedirect option callback before redirections', (done) => {
      ...
      const beforeRedirectCallback = function(redirectMethod, statusCode, location, redirectOptions) {
        ...
        redirectOptions.headers = {'x-test': 'Modified'};
      };
      server.listen(0, () => {
        Wreck.request('get', 'http://localhost:' + server.address().port, {
          redirects: 5,
    ...
    it('pooling can be disabled by setting agent to false', (done) => {
      ...
      server.listen(0, () => {
        Wreck.request('get', 'http://localhost:' + server.address().port, {
          ...
          Wreck.request('get', 'http://localhost:' + server.address().port + '/thatone', {
            agent: false,
            timeout: 15
          }, (err, innerRes) => {
            expect(err).to.not.exist();
