...
var stream_assert = require('./stream-assert');
var gulp = require('gulp');
var gulp_jspm = require('../index');
...
describe('gulp_jspm()', function() {
  ...
  it('should not throw any errors', function(done) {
    this.timeout(4000);
    stream = stream.pipe(through2.obj(function(file, enc, cb) {
      file.sourceMap = true;
      this.push(file);
      cb();
    })).pipe(gulp_jspm()).pipe(stream_assert.done(done));
  });
  it('should return one file', function(done) {
    stream = stream.pipe(stream_assert.length(1)).pipe(stream_assert.done(done));
  });
  it("should return the bundle's content", function(done) {
    stream = stream.pipe(stream_assert.test(function(bundle_file) {
  ...
  it("should support arithmetics", function(done) {
    gulp.src(script_path).pipe(gulp_jspm({arithmetic: '   - message  '})).pipe(stream_assert.test(function(bundle_file) {
      assert(bundle_file.contents);
      assert(/System\.register\("main.js".*message/.test(bundle_file.contents));
      assert(!/System\.register\("message.js"/.test(bundle_file.contents.toString()));
    })).pipe(stream_assert.done(done));
