...
(function(Buffer) {
  ...
  var projectName = require('./package.json!systemjs-json').name;
  module.exports = function(opts) {
    return through.obj(function(file, enc, cb) {
      ...
      opts = opts || {};
      do_bundle(file, opts).then(function(infos) {
        setTimeout(function() {
  ...
  function do_bundle(file, opts) {
    ...
    return Promise.resolve({
      ...
      return (get_paths(file.base)).then(function(paths) {
      ...
      return (Promise.promisify(temp.open)('gulp-jspm__build.js')).then(function(temp_file) {
      ...
      return Promise.resolve(jspm[method](jspm_input, jspm_output, jspm_opts)).then(function() {
        info_log('jspm.' + method + '() called', infos);
        return infos;
      });
    }).then(function(infos) {
      return Promise.all([fs.readFileAsync(infos.bundle.path).then(function(file_content) {
        if (file.sourceMap) {
      ...
      })].concat(!file.sourceMap ? [] : (fs.readFileAsync(infos.bundle.path + '.map').then(function(file_content) {
      ...
      var bundle_file = infos.bundle.vinyl_file = new File({
        base: file.base,
        path: (function() {
      ...
      if (file.sourceMap) {
        ...
        bundle_file.sourceMap.sources = bundle_file.sourceMap.sources.map(function(relative_to_temp) {
  ...
  function get_paths(directory) {
    return new Promise(function(resolve) {
      ...
      }).launch({cwd: directory}, function(env) {
        resolve({
          baseURL: (function() {
            if (env.configBase) {
              var package_info = require(env.configPath);
              if (package_info && package_info.jspm && package_info.jspm.directories && package_info.jspm.directories.baseURL) {
  ...
  function info_log(message, infos) {
    if (info_log.enable) {
      console.log(projectName + ':', message + ',', 'collected information at this point;\n', infos);
    }
  }
